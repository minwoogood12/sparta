...
class Trainer:
    ...
    def _step(
        ...
        #Batch = 1 
        images = batch.img_batch.squeeze(1) 
        ...
        images = images * std + mean          # 0~1 범위 복원
        images = (images * 255.0).clamp(0, 255).byte()  # 0~255, uint8
        '''
        import os
        ...
        # images: [T, C, H, W]  (batch_size=1 가정)
        for t in range(images.size(0)):
            save_path = os.path.join(save_dir, f"frame_{t:03d}.png")
            save_image(images[t].float() / 255.0, save_path)  # 0~1 범위로 변환 후 저장
            print(f"Saved: {save_path}")
        '''

        k_size = 3
        downsampled_images = F.avg_pool2d(images.float(), kernel_size=4, stride=4, padding=0)
